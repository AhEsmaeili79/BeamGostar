...
def get_menu_structure():
    ...
    """
    # Fetch active menus
    menus = Menu.objects.filter(is_active=True).prefetch_related('submenus__subitems')
    
    menu_structure = []
    for menu in menus:
        # Filter active submenus
        submenus = menu.submenus.filter(is_active=True).order_by('order')  # Filter active submenus
            ...
            'submenus': [
                {
                    ...
                    'subitems': [
                        ...
                        } for subitem in submenu.subitems.filter(is_active=True).order_by('submenu__order')  # Ensure only active subitems
                    ]
